...
function AccordionIcon<T extends React.ElementType>({ className, as, ...rest }: PolymorphicComponentProps<T>) {
...
type AccordionArrowProps = React.HTMLAttributes<SVGSVGElement> & {
    size?: number;
    openIcon?: HugeiconsIconProps["icon"];
    closeIcon?: HugeiconsIconProps["icon"];
};

// open/close
function AccordionArrow({
    className,
    openIcon = PlusSignIcon,
    closeIcon = MinusSignIcon,
    ...rest
}: AccordionArrowProps) {
    return (
        <>
